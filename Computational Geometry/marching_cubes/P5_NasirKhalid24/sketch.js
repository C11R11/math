...
function setup() {
  ...
  console.log("surface", surface, Object.keys(surface).length)
  console.log("map", surface[`${item[0]}, ${item[1]}, ${item[2]}`])
  console.log("map_>", map(surface[`${item[0]}, ${item[1]}, ${item[2]}`], -1, 1, 0, 255))
  console.log("asdsa->", stroke(map(surface[`${item[0]}, ${item[1]}, ${item[2]}`], -1, 1, 0, 255)))
...
function basics(){
  ...
  noFill();
  translate(0, 0, 0);
  box(gridLimit*3, gridLimit*3, gridLimit*3);
  ...
  let i = 0
  //drawing the points (different collor for the surface ones)
  for (let item of points){
    if(surface[`${item[0]}, ${item[1]}, ${item[2]}`] == 1){
    push();
...
function marching(){
  ...
  //boxIndex = apointPossitive

  console.log(`Marching boxIndex ${boxIndex} `)
  currentBox = boxPositions[boxIndex];
  console.log("currentBox ", boxPositions[boxIndex])
  
  vertexCoords = cubeVertices.map(item => {
    return [ item[0]+currentBox[0], item[1]+currentBox[1], item[2]+currentBox[2] ]
  });

  console.log("vertexCoords ", vertexCoords)
  ...
  console.log("pointsConnect", pointsConnect)
...
//here all the points connected are drawn into the scene directly
function surfaces(){
  for (const key in planes) {
    if (Object.hasOwnProperty.call(planes, key)) {
      const element = planes[key];

      //console.log(" planes[key]",  planes[key])
      ...
      for(var i=0; i<element.length; i+=3){
        ...
        [element[i+1][0], element[i+1][1], element[i+1][2]],
        [element[i+2][0], element[i+2][1], element[i+2][2]]
        ) */
        endShape();
